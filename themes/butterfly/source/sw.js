...
const cache_url_list = [/(http:\/\/|https:\/\/)rmt\.ladydaily\.com/g, /(http:\/\/|https:\/\/)rmt\.dogedoge\.com/g]
const blog = {
    local: 0,
    origin: ["blog.slqwq.cn"],
    plus: ["blog.slqwq.cn", "gcore.blog.slqwq.cn","deno.blog.slqwq.cn","cloudfront.blog.slqwq.cn","hajeekns-blog.pages.dev","vercel.blog.slqwq.cn"],
};
...
const handle = async function (req) {
    ...
    if (pathname.match('/cdn-cgi/')) return new Response(null, {
        status: 308
    })
    const path = pathname.split('?')[0]
    const query = q => urlObj.searchParams.get(q)
    let urls = []
    let msg = JSON.parse(await db.read('msg')) || (async () => {
        await db.write('msg', '[]');
        return '[]'
    })()
...
const lfetch = async (urls, url) => {
    ...
    let results = Promise.any(urls.map(async urls => {
        return new Promise(async (resolve, reject) => {
            ...
            }).then(PauseProgress).then(async res => {
...
const handlecgi = async (req) => {
    ...
    const MSG_HTML = (() => {
        ...
        for (let i in msg_init.reverse()) {
            u += msg.replace(/<!--NAME-->/g, msg_init[i].name).replace(/<!--TIME-->/g, msg_init[i].time).replace(/<!--INFO-->/g, msg_init[i].info)
        }
        console.log(u)
        return u
    })()
    ...
    switch (query('dash')) {
        case 'test':
            ...
            return new Response(dash_main.replace(/<!--TEST_ACTIVE-->/g, ' active').replace(/<!--SECTION-->/g, test_html), {
                headers: {
        ...
        case 'cache':
            ...
            return new Response(dash_main.replace(/<!--CACHE_ACTIVE-->/g, ' active').replace(/<!--SECTION-->/g, cache_content_html).replace(/<!--CACHE_COUNT-->/g, CACHE_COUNT).replace(/<!--CACHE_SIZE_COUNT-->/g, intelligent_size(CACHE_SIZE_COUNT)).replace(/<!--INFO-->/g, (() => {
                o = ""
                for (var i in cache_list) {
                    o += cache_section_html.replace(/<!--URL-->/g, cache_list[i].url).replace(/<!--SIZE-->/g, intelligent_size(cache_list[i].size)).replace(/<!--TIME-->/g, cache_list[i].time).replace(/<!--CACHE_ROUND-->/g, (cache_list[i].size / CACHE_SIZE_COUNT * 100).toFixed(2))
                }
                return o
            })()), {
                status: 200,
        ...
        default:
            ...
            return new Response(dash_main.replace(/<!--HOME_ACTIVE-->/g, 'active').replace(/<!--SECTION-->/g, content).replace(/<!--HIT_COUNT-->/g, HIT_COUNT).replace(/<!--HIT_SIZE_COUNT-->/g, intelligent_size(HIT_COUNT_SIZE)).replace(/<!--INFO-->/g, (() => {
                t = ""
                for (var i in HIT_ALL) {
                    t += info.replace(/<!--DOMAIN-->/g, i).replace(/<!--HIT_COUNT-->/g, HIT_ALL[i]).replace(/<!--HIT_SIZE_COUNT-->/g, intelligent_size(HIT_ALL_SIZE[i])).replace(/<!--HIT_ROUND-->/g, (HIT_ALL_SIZE[i] / HIT_COUNT_SIZE * 100).toFixed(2)).replace(/<!--TYPE-->/g, (() => {
